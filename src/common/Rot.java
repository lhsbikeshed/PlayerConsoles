 ...
 * generic methods: {@link #applyTo(PVector) applyTo(PVector)} and
 * {@link #applyInverseTo(PVector) applyInverseTo(PVector)}.
 * </p>
 * 
 * <p>
 ...
 * provides the methods: {@link #applyTo(PVector) applyTo(PVector)} and
 * {@link #applyInverseTo(PVector) applyInverseTo(PVector)}.
 * </p>
 * 
 * <p>
 ...
 * </p>
 * 
 * @version $Revision: 772119 $ $Date: 2009-05-06 05:43:28 -0400 (Wed, 06 May
 *          2009) $
 * 
 * @see PVector
 * @see RotOrder
...
public class Rot implements VectorConstants, Serializable {
	...
	public static final Rot slerp(Rot a, Rot b, float t, boolean allowFlip) {
		...
		return new Rot(c1 * a.q0 + c2 * b.q0, c1 * a.q1 + c2 * b.q1, c1 * a.q2
				+ c2 * b.q2, c1 * a.q3 + c2 * b.q3, false);
	}
	...
	public Rot(float q0, float q1, float q2, float q3,
		...
		if (needsNormalization) {
			float inv = 1.0f / (float) Math.sqrt(q0 * q0 + q1 * q1 + q2 * q2
					+ q3 * q3);
			q0 *= inv;
			q1 *= inv;
			q2 *= inv;
	...
	public Rot(float[][] m, float threshold) {
		...
		if ((m.length != 3) || (m[0].length != 3) || (m[1].length != 3)
			...
			q0 = 1;
			q1 = q2 = q3 = 0;
			System.out.println(Messages.build(
					"a {0}x{1} matrix cannot be a rotation matrix", m.length,
					m[0].length));
			return;
		}
		...
		if (ort == null) {
			...
			q1 = q2 = q3 = 0;
			System.out.println(Messages.build(
					"unable to orthogonalize matrix in {0} iterations", 10));
			return;
		}
		...
		if (det < 0.0f) {
			...
			System.out
					.println(Messages
							.build("the closest orthogonal matrix has a negative determinant {0}",
									det));
			return;
		}
	 ...
	 * angle to this constructor, and then {@link #applyTo(PVector) apply} the
	...
	public Rot(PVector axis, float angle) {
		...
		if (norm == 0) {
			...
			q1 = q2 = q3 = 0;
			System.out.println(Messages.build(
					"The axis vector {0} has no magnitude!!", axis));
			return;
		}
	...
	public Rot(PVector u1, PVector u2, PVector v1, PVector v2) {
		...
		float beta = (float) Math.sqrt((u2u2 - u1u2 * coeffU)
				/ (v2v2 - v1v2 * coeffV));
		...
		float dy2 = v2y - u2.y;
		float dz2 = v2z - u2.z;
		PVector k = new PVector(dy1 * dz2 - dz1 * dy2, dz1 * dx2 - dx1 * dz2,
				dx1 * dy2 - dy1 * dx2);
		float c = k.x * (u1y * u2z - u1z * u2y) + k.y * (u1z * u2x - u1x * u2z)
				+ k.z * (u1x * u2y - u1y * u2x);
		...
		if (c == 0) {
			...
			float dy3 = v3y - u3y;
			float dz3 = v3z - u3z;
			k = new PVector(dy1 * dz3 - dz1 * dy3, dz1 * dx3 - dx1 * dz3, dx1
					* dy3 - dy1 * dx3);
			c = k.x * (u1y * u3z - u1z * u3y) + k.y * (u1z * u3x - u1x * u3z)
					+ k.z * (u1x * u3y - u1y * u3x);

			if (c == 0) {
				// the (q1, q2, q3) vector is aligned with u1:
				// we try (u2, u3) and (v2, v3)
				k = new PVector(dy2 * dz3 - dz2 * dy3, dz2 * dx3 - dx2 * dz3,
						dx2 * dy3 - dy2 * dx3);
				c = k.x * (u2y * u3z - u2z * u3y) + k.y
						* (u2z * u3x - u2x * u3z) + k.z
						* (u2x * u3y - u2y * u3x);
		...
		k = new PVector(uRef.y * q3 - uRef.z * q2, uRef.z * q1 - uRef.x * q3,
				uRef.x * q2 - uRef.y * q1);
		c = PVector.dot(k, k);
	...
	public Rot applyInverseTo(Rot r) {
		return new Rot(-r.q0 * q0 - (r.q1 * q1 + r.q2 * q2 + r.q3 * q3), -r.q1
				* q0 + r.q0 * q1 + (r.q2 * q3 - r.q3 * q2), -r.q2 * q0 + r.q0
				* q2 + (r.q3 * q1 - r.q1 * q3), -r.q3 * q0 + r.q0 * q3
	...
	public float getAngle() {
		if ((q0 < -0.1) || (q0 > 0.1)) {
			return 2 * (float) Math
					.asin(Math.sqrt(q1 * q1 + q2 * q2 + q3 * q3));
		} else if (q0 < 0) {
	 ...
	 * @return an array of three angles, in the order specified by the set or
	 *         null if angle singularity found.
	 */
	public float[] getAngles(RotOrder order) {
		...
		if (order == RotOrder.XYZ) {
			...
			return new float[] { (float) Math.atan2(-(v1.y), v1.z),
					(float) Math.asin(v2.z), (float) Math.atan2(-(v2.y), v2.x) };
			...
			return new float[] { (float) Math.atan2(v1.z, v1.y),
					-(float) Math.asin(v2.y), (float) Math.atan2(v2.z, v2.x) };
			...
			return new float[] { (float) Math.atan2(v1.x, v1.z),
					-(float) Math.asin(v2.z), (float) Math.atan2(v2.x, v2.y) };
			...
			return new float[] { (float) Math.atan2(-(v1.z), v1.x),
					(float) Math.asin(v2.x), (float) Math.atan2(-(v2.z), v2.y) };
			...
			return new float[] { (float) Math.atan2(-(v1.x), v1.y),
					(float) Math.asin(v2.y), (float) Math.atan2(-(v2.x), v2.z) };
			...
			return new float[] { (float) Math.atan2(v1.y, v1.x),
					-(float) Math.asin(v2.x), (float) Math.atan2(v2.y, v2.z) };
			...
			return new float[] { (float) Math.atan2(v1.y, -v1.z),
					(float) Math.acos(v2.x), (float) Math.atan2(v2.y, v2.z) };
			...
			return new float[] { (float) Math.atan2(v1.z, v1.y),
					(float) Math.acos(v2.x), (float) Math.atan2(v2.z, -v2.y) };
			...
			return new float[] { (float) Math.atan2(v1.x, v1.z),
					(float) Math.acos(v2.y), (float) Math.atan2(v2.x, -v2.z) };
			...
			return new float[] { (float) Math.atan2(v1.z, -v1.x),
					(float) Math.acos(v2.y), (float) Math.atan2(v2.z, v2.x) };
			...
			return new float[] { (float) Math.atan2(v1.x, -v1.y),
					(float) Math.acos(v2.z), (float) Math.atan2(v2.x, v2.y) };
			...
			return new float[] { (float) Math.atan2(v1.y, v1.x),
					(float) Math.acos(v2.z), (float) Math.atan2(v2.y, -v2.x) };
		}
	}
	...
	private float[][] orthogonalizeMatrix(float[][] m, float threshold) {
		...
		while (++i < 11) {
			...
			fn1 = corr00 * corr00 + corr01 * corr01 + corr02 * corr02 + corr10
					* corr10 + corr11 * corr11 + corr12 * corr12 + corr20
					* corr20 + corr21 * corr21 + corr22 * corr22;
