...
public class BootDisplay extends Display {
	...
	String[] fileNames = { "kernel32.sys", "PilotIOController.sys",
			"SplineReticulator.so" };
	int nextFail = 100;
	...
	public void draw() {
		...
		if (bootCount < 100) {
			...
			int bootLen = (int) PApplet.map(bootCount, 0, 100, 0,
					bootText.length);
	...
	public void oscMessage(OscMessage theOscMessage) {
		if (theOscMessage.checkAddrPattern("/system/boot/diskInsert") == true) {
			boolean correct = theOscMessage.get(0).intValue() == 1 ? true
					: false;
			if (correct) {
				curFile++;
				if (curFile < 3) {
				...
				bannerSystem.cancel();
				bannerSystem.setText("INCORRECT DISK. PLEASE INSERT DISK "
						+ filesToReplace[curFile]);
				bannerSystem.displayFor(360000);
			}

		} else if (theOscMessage
				.checkAddrPattern("/system/boot/justFuckingBoot")) {
			// sometimes boots get stuck, this will skip it and power the ship
			// on
			brokenBoot = false;
			bannerSystem.cancel();
			bootCount = 400;
