...
public class WarpDisplay extends Display {
  ...
  public WarpDisplay(PlayerConsole parent) {
    ...
    overlayImage = parent.loadImage("common/warpScreen/hyperfailoverlay.png");
    ...
    for (int i = 0 ; i < 50; i++) {
      stars[i] = new Point((int)parent.random(parent.width + 100), gridY + (int)parent.random(400));
    }  
  ...
  void drawGrid() {
    ...
    if (sceneStart + 5000 > parent.millis() && warpingIn) {
      warpAmount = parent.map(parent.millis() - sceneStart, 0f, 5000f, 0.0f, 1.0f);
    } 
    else {
      if(warpingIn){
        screenFlashTimer = 40;
    ...
    if(warpingOut){
      warpAmount = parent.map(parent.millis() - exitStartTime, 0f, 8000f, 1.0f, 0.0f);
    ...
    for (int i = 0; i < stars.length; i++) {
      ...
      warpGrid.rect(stars[i].x, stars[i].y, (float) (40 * (warpAmount + 0.1)), 1);
    }
    warpGrid.strokeWeight(1);
    warpGrid.stroke(255, 0, 0);
    warpGrid.noFill();
    for (int y = 0; y < gridHeight - 1; y++) {
      for (int x = 0; x < gridWidth - 1; x++) {
        ...
        //calc sin offset
        float s = 10 - Math.abs(y - 10) - ((y - 10) * (y - 10)) * warpAmount * 20.0f;
        // s = sin(s) * (mouseX / 100.0f);
        float sNext = 10 - Math.abs(y - 9) - ((y - 9) * (y - 9)) * warpAmount * 20.0f ;
        ...
        warpGrid.stroke(0, 0, parent.map(parent.sin((float) ((parent.millis() / 250.0f) + (x * 0.1))), -1.0f, 1.0f, 120, 255));
  ...
  public void draw() {
    ...
    if(thisIsFail){
      ...
      planetX = (int)parent.lerp(parent.width, 637, (parent.millis() - sceneStart) / 20000.0f);
      planetScale = parent.lerp(0.6f, 1.0f, (parent.millis() - sceneStart) / 20000.0f);
    ...
    if (timeRemaining >= 0.0f ) {
      if( parent.millis() - lastUpdate < 275f){
        float t = parent.lerp(lastTimeRemaining, timeRemaining, (parent.millis() - lastUpdate) / 250.0f); 
    ...
    if(screenFlashTimer > 0){
      screenFlashTimer--;
      float alpha = parent.map(screenFlashTimer, 40, 0, 255,0);
