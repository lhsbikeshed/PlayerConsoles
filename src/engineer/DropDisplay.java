...
public class DropDisplay extends Display {
  ...
  public DropDisplay(PlayerConsole parent) {
    ...
    jumpOverlayImage = parent.loadImage("engineerconsole/emergencyjump.png");
    jumpEnableOverlay = parent.loadImage("engineerconsole/jumpEnableOverlay.png");
    structFailOverlay = parent.loadImage("engineerconsole/structuralFailure.png");
    dropFailOverlay = parent.loadImage("engineerconsole/dropFailOverlay.png");
    ...
    if (state == STATE_INST) {
      ...
      for (int i = 0; i < 5; i ++) {
        ...
        if (i < currentCable) {
          ...
          if(i == currentCable){
              int c = (int)parent.map(parent.sin(parent.millis() / 100.0f), -1.0f, 1.0f, 0, 255);  
            parent.fill(c,c,0);
          } else {
      ...
      else if (authDisplayTime +2500 > parent.millis() && authResult == true) {
        state = STATE_CODEOK;
      } 
      ...
      if (state == STATE_CODEOK) {
        ...
        parent.rect(125, 469, parent.map(chargePercent, 0, 1.0f, 0, 480), 48);
  ...
  public void start() {
    ...
    //randomise the order
    for (int i = 4; i > 0; i--) {
      int rand = parent.floor(parent.random(i+1));
      if (rand != i) {
        int t = cableOrder[i];
