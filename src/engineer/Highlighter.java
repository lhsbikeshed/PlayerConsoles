	...
	public void update() {
		if (running) {
			...
			if (startTime + time < parent.millis()) {
				...
				w = (int) PApplet.lerp(startSize.x, endSize.x, t);
				h = (int) PApplet.lerp(startSize.y, endSize.y, t);
			}
			parent.noFill();
			int r = (int) PApplet.map(PApplet.sin(parent.millis() / 100.0f),
					-1.0f, 1.0f, 120, 255);
			parent.stroke(r, 0, 0);
			parent.strokeWeight(4);
			for (int i = 0; i < 4; i++) {
				parent.rect(centre.x - w / 2 - i * 5, centre.y - h / 2 - i * 5,
						w + i * 10, h + i * 10);
			}
		}
	}
