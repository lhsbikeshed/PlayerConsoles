...
public class Sprite {
  ...
  PApplet parent;
  public Sprite(PApplet parent, PImage sheet, int w, int h, int numFramesX, int numFramesY){
	  this.parent = parent;
    frameW = w;
    frameH = h;
    ...
    for(int xp = 0; xp < numFramesX; xp++){
      for(int yp = 0; yp < numFramesY; yp++){
        ...
        frames[fNum] = parent.createImage(frameW, frameH, PApplet.ARGB);
  ...
  public void draw(int x, int y){
    parent.image(frames[currentFrame], x - xCentre, y - yCentre, drawWidth, drawHeight);
  }
