...
public abstract class SubSystem {
  ...
  public void smash() {
    ...
    ((EngineerConsole)parent).addHighlight( new Highlighter(parent, new PVector(pos.x + getImg().width/2, pos.y + getImg().height/2), new PVector(getImg().width * 5, getImg().height*5), new PVector(getImg().width, getImg().height), 1000, 2000));
    health = 0;
    //isBroken = true;
    createFailure();
  }
  ...
  public void draw() {
    ...
    if (isBroken()) {
      ...
      parent.image(getImg(), pos.x + parent.random(4) - 2, pos.y + parent.random(4) - 2);
    } 
    else {
