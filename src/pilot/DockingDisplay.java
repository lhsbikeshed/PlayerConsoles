...
public class DockingDisplay extends Display {
  ...
  public void draw() {
    distanceScale = parent.map(distance, 0, 350, 0.5f, 4.0f);
    ...
    parent.noFill();
    for (int i = 1; i < 4; i++) {
      parent.ellipse(parent.width/2, parent.height/2, i * 150, i * 150);
    }
    ...
    if (lockingState != NO_SIGNAL) {
      ...
      lerpPos.x = parent.lerp(lastShipPos.x, shipPos.x, (parent.millis() - lastPosUpdate) / 200.0f);
      lerpPos.y = parent.lerp(lastShipPos.y, shipPos.y, (parent.millis() - lastPosUpdate) / 200.0f);

      int screenX = (int)parent.map(lerpPos.x, 45.0f, -45.0f, 0f, parent.width);
      int screenY = (int)parent.map(lerpPos.y, 45.0f, -45.0f, 0f, parent.height);
    ...
    parent.text("Speed: " + (int)parent.getShipState().shipVelocity, 58, 183);
