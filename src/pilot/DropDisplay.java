...
public class DropDisplay extends Display {
  ...
  public DropDisplay(PlayerConsole parent) {
    ...
    structFailOverlay = parent.loadImage("pilotconsole/structuralFailure.png");
  ...
  public void draw() {
    ...
    float alt = parent.lerp(lastAltitude, altitude, (parent.millis() - lastUpdate) / 250.0f);
    ...
    if(fireVec.z > 0){
      ...
      parent.image(fireballImg, 643 + randX, 231 + randY, fireballImg.width / 2, fireballImg.height / 2);
    }
    if(fireVec.z < 0){
      ...
      parent.scale(1, -1);
      parent.image(fireballImg, 0, 0, fireballImg.width / 2, fireballImg.height / 2);
      parent.popMatrix();
    }
    ...
    if(turbulenceTime < parent.millis() && parent.millis() < turbulenceTime + 1500){
