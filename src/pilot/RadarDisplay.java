...
public class RadarDisplay extends Display {
  ...
  public void drawGuides() {
    ...
    //left is at 28,326
    if (guideVector.x < 0) {
      parent.tint(255, 255, 255, (int)parent.map(Math.abs(guideVector.x), 0, 1, 0, 255));

      parent.image(guideArrow, 28, 326);
    } 
    else {
      parent.tint(255, 255, 255, (int)parent.map(guideVector.x, 0, 1, 0, 255));
    ...
    parent.pushMatrix();
    if (guideVector.y < 0) {
      parent.tint(255, 255, 255, (int)parent.map(Math.abs(guideVector.y), 0, 1, 0, 255));
      ...
      parent.tint(255, 255, 255, (int)parent.map(guideVector.y, 0, 1, 0, 255));
  ...
  public void drawRadar() {
    ...
    parent.rotateX(parent.radians(345)); //326
...
//noFill();
    ...
    parent.sphere(1);
    parent.fill(0, 0, 255);
    zoomLevel = parent.map(maxDist, 0f, 5000f, 1.8f, 0.2f);
    ...
    synchronized(lock) {
      for (int i = 0; i < 100; i++) {
        ...
        if (rItem.active == true) {
          ...
          newPos.x = parent.lerp(rItem.lastPosition.x, rItem.position.x, (parent.millis() - rItem.lastUpdateTime) / 250.0f );
          newPos.y = parent.lerp(rItem.lastPosition.y, rItem.position.y, (parent.millis() - rItem.lastUpdateTime) / 250.0f);
          newPos.z = parent.lerp(rItem.lastPosition.z, rItem.position.z, (parent.millis() - rItem.lastUpdateTime) / 250.0f);
          ...
          parent.line(-newPos.x, 0, newPos.z, -newPos.x, -newPos.y, newPos.z);
          //circle at base       
          parent.pushMatrix();
          ...
          rItem.screenPos.x = parent.screenX(-newPos.x, -newPos.y, newPos.z);
          rItem.screenPos.y = parent.screenY(-newPos.x, -newPos.y, newPos.z);
          parent.translate(-newPos.x, -newPos.y, newPos.z);    
          parent.noStroke();
          int alpha = (int)parent.lerp(255, 0, (parent.millis() - rItem.lastUpdateTime) / 250.0f);
        ...
        if (rItem.active) {
          ...
          if (rItem.distance > 1000) {
            ...
            for (int c = 0; c < (int)parent.random(3,s.length()); c++) {
              s.setCharAt( (int)parent.random(0, s.length()), (char)parent.random(0, 255));
            }

            parent.fill(40);
            parent.text(s.toString(), rItem.screenPos.x + 5, rItem.screenPos.y + 10);
          } 
          else {
            parent.fill(rItem.displayColor);
            parent.text(rItem.name, rItem.screenPos.x + 5, rItem.screenPos.y + 10);
          }
          // textFont(font, 10);
          ...
          if (rItem.targetted) {
            ...
            parent.rotateZ(parent.radians( (parent.millis() / 10.0f) % 260));
            ...
            parent.line(rItem.screenPos.x + midX, 190, rItem.screenPos.x, rItem.screenPos.y);
          }
          ...
          //stroke(255,255,0);
          if (f != null && f > 0.0) {
            int radius = (int)parent.map(parent.millis() % 3000, 0, 3000, 0, 100);
            int alpha = (int)parent.map(parent.millis() % 3000, 0, 3000, 255, 0);
            parent.fill(255, 255, 0, alpha);
            parent.ellipse(rItem.screenPos.x, rItem.screenPos.y, radius, radius);

            radius = (int)parent.map((parent.millis() + 1500) % 3000, 0, 3000, 0, 100);
            alpha = (int)parent.map((parent.millis() + 1500)  % 3000, 0, 3000, 255, 0);
            parent.fill(255, 255, 0, alpha);
            parent.ellipse(rItem.screenPos.x, rItem.screenPos.y, radius, radius);
          }
        }
    ...
    if (targetted != null) {
      ...
      float yRotation = (270 + parent.degrees(heading(tempVec.x, tempVec.y)) )% 360;
      if (yRotation > 0 && yRotation < 180) {  //right hand side of ship
        guideVector.x = -parent.map(yRotation, 0, 180, 0, 1);
      } 
      else {
        guideVector.x = -parent.map(yRotation, 180, 360, -1, 0);
      }
      tempVec.x = targetted.position.z;
      tempVec.y = targetted.position.y;

      float xRotation =  parent.degrees(heading(tempVec.x, tempVec.y));
      if (xRotation > -90 && xRotation < 0) {
        guideVector.y = -parent.map(xRotation, -90, 0, 1, 0);
      } 
    ...
    parent.text("Sector (" + sectorX + "," + sectorY + "," + sectorZ + ")", 41, 740);
  ...
  public void oscMessage(OscMessage theOscMessage) {
    ...
    if (theOscMessage.checkAddrPattern("/radar/update")) {
      synchronized(lock) {
        ...
        radarList[rId].displayColor = UsefulShit.makeColor (   (int)(Float.parseFloat(splitColour[0]) * 255), 
