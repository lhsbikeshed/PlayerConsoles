...
public class WeaponsConsole extends Display {
  ...
  void drawSideBar() {
    ...
    case SCAN_OK:
      if(parent.globalBlinker) parent.text("LOCKED", 758, 737);
    ...
    //draw hull damage
    parent.tint( (int)parent.map(parent.getShipState().hullState, 0, 100, 255, 0), (int)parent.map(parent.getShipState().hullState, 0, 100, 0, 255), 0);
  ...
  void drawTargets() {
    ...
    synchronized(targets) {
      for (int i = targets.size() - 1; i >= 0; i--) {
        ...
        float lerpX = parent.lerp(t.lastPos.x, t.pos.x, (parent.millis() - t.lastUpdateTime ) / 250.0f);
        float lerpY = parent.lerp(t.lastPos.z, t.pos.z, (parent.millis() - t.lastUpdateTime ) / 250.0f);
        float lerpZ = parent.lerp(t.lastPos.y, t.pos.y, (parent.millis() - t.lastUpdateTime ) / 250.0f);
        ...
        float x = 364 + parent.lerp(lp.x, p.x, (parent.millis() - t.lastUpdateTime ) / 250.0f);
        ;
        float y = 707 + parent.lerp(lp.y, p.y, (parent.millis() - t.lastUpdateTime ) / 250.0f);
        ;
        ...
        if (t.pos.mag() < sensorRanges[sensorPower - 1]) {    //grey it out if its outside of sensor range, if not then draw
          ...
          if (f != null && f > 0.0f) {
            ...
            int maxSize = 70;
            int size = (int)parent.map( parent.millis() % 2000, 0, 2000, 0, maxSize);
            parent.noFill();
            parent.strokeWeight(2);
            parent.stroke(0, 128, 255, parent.map(size, 0, maxSize, 255, 0));
            parent.ellipse(x, y, size, size);
            size = (int)parent.map( (parent.millis() + 1000) % 2000, 0, 2000, 0, maxSize);
            parent.stroke(0, 128, 255, parent.map(size, 0, maxSize, 255, 0));
            parent.ellipse(x, y, size, size);
          }
        ...
        if (t.scanCountDown > 0) {
          ...
          parent.rotate(parent.radians((parent.millis() / 10.0f) % 360));
          parent.noFill();
          parent.stroke(255, 255, 0);
          float scale = parent.map(t.scanCountDown, 100, 0, 10, 1);
          parent.rect(-15 * scale, -15* scale, 30 * scale, 30 * scale);
          parent.popMatrix();
...
ConsoleLogger.log(this, theOscMessage.addrPattern());
    ...
    if (theOscMessage.checkAddrPattern("/control/subsystemstate") == true) {
      ...
      synchronized(targets) {
        ...
        if (newTarget) {
          ...
          t.lastPos.y = y;
          t.lastPos.z = z;
          t.randomAngle = parent.map(parent.random(100), 0, 100, 4.2f, 5.23f);
        } 
        else {
  ...
  public class TargetObject implements Comparable<TargetObject> {
    ...
    public TargetObject() {
      //4.2 - 5.23 
      randomAngle = parent.map(parent.random(100), 0, 100, 4.2f, 5.23f);
    }
